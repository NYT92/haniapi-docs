...
For Non Self host User : Before using the AuthAPI, Read this [Info](docs/info#usage) before you can use it.

## Update

<Callout>
    UPDATE : We have added the new authentication methods and update the auth routes.
</Callout>


## /auth/login

To login your Hanime.tv Account :
> /auth/login?email=YOUR_HANIME_EMAIL&password=YOUR_HANIME_PASSWORD
...
To summary important info of your account :
> /auth/login/summary?email=YOUR_HANIME_EMAIL&password=YOUR_HANIME_PASSWORD
...
To get a coins from Hanime without installing app or clicking ads
> /auth/login/coins?email=YOUR_HANIME_EMAIL&password=YOUR_HANIME_PASSWORD
...
<Callout type="warning" emoji="⚠️">
    This API CANNOT give you unlimited coins. The Hanime Server API allowed 3 hours per each request. !!!
</Callout>

## /auth/login/body

To login to your account with JSON body type :
...
The results should be the same as the [Login](#authlogin)


## /auth/coin/body

To get coins from your account with JSON body type :
...
The results should be the same as the [Coins](#authlogincoins)

## /auth/login/getsession

You can use /getsession to get your hanime account session_token for [Get Premium](/getapi#getvideopremium).  
Becareful with your account session_token and never give anyone the token because thief might have used to hacked your account.   
It will expire for 30 days or when the user requesting new session_token.

> /auth/login/getsession?email=YOUR_HANIME_EMAIL&password=YOUR_HANIME_PASSWORD
